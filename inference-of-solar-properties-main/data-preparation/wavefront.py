...
def scaled_wavefront(wavefront, npix_wavefront=None):
  """
  Transform a wavefront to a given size
  
  Args:
      wavefront (TYPE): wavefront
      npix_wavefront (int, optional): number of extra pixels
  
  Returns:
      real: PSF
  """

  if (npix_wavefront is None):
    return wavefront

  npix = len(wavefront)

  if (npix_wavefront >= npix):

    if (npix_wavefront % 2 != 0):
      npix_wavefront += 1
    wfbig = numpy.zeros((npix_wavefront,npix_wavefront),dtype='d')

    half = int((npix_wavefront - npix)/2)
...
def psf(aperture, wavefront, overfill=1, npix_wavefront=None):
  """
